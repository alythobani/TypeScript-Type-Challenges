...
type Flatten<M extends unknown[][], Acc extends unknown[] = []> = M extends [
...
type Fill<N extends number, V, Acc extends V[] = []> = Acc["length"] extends N
  ? Acc
  : Fill<N, V, [...Acc, V]>;

type Square<N extends number, AbsN extends number = Abs<N>> = Flatten<
  Fill<AbsN, Fill<AbsN, 0>>
> extends { length: infer L }
  ? L
  : never;

type Abs<N extends number> = `${N}` extends `-${infer AbsN extends number}`
  ? AbsN
  : N;
