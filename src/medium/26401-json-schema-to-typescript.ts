...
type Merge<A, B> = Omit<A & B, never>;

type JSONSchema2TS<T> = T extends { type: infer t }
  ? t extends "string"
    ...
    ? T extends { properties: infer P }
      ? T extends { required: infer R extends string[] }
        ? Merge<
            { [k in Exclude<keyof P, R[number]>]?: JSONSchema2TS<P[k]> },
            {
              [k in keyof P & R[number]]: JSONSchema2TS<P[k]>;
            }
          >
        : { [k in keyof P]?: JSONSchema2TS<P[k]> }
      : Record<string, unknown>
    : t extends "array"
    ? T extends { items: infer ItemType }
      ? JSONSchema2TS<ItemType>[]
      : unknown[]
    : never
  : never;
