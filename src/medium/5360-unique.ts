...
type Contains<T, V> = T extends [infer First, ...infer Rest]
  ? Equal<First, V> extends true
    ? true
    : Contains<Rest, V>
  : false;

type Unique<T, Result extends unknown[] = []> = T extends [
  infer First,
  ...infer Rest
]
  ? Unique<
      Rest,
      Contains<Result, First> extends true ? Result : [...Result, First]
    >
  : Result;
...
type f1 = Unique<[1, 1, 2, 2, 3, 3]>;
type f4 = Unique<[string, number, 1, "a", 1, string, 2, "b", 2, number]>;
