...
// Note - I didn't come up with this solution myself, I looked at the solutions available at the
// GitHub repo: https://github.com/type-challenges/type-challenges/blob/main/questions/00730-hard-union-to-tuple/README.md
// I think this is super hacky but interesting that it works due to the way TypeScript handles function overloads.

type UnionToTuple<T, L = LastInUnion<T>> = [T] extends [never]
  ? []
  : [L, ...UnionToTuple<Exclude<T, L>>];
type LastInUnion<T> = InferArg<IntersectedFnSignatures<T>>;
type IntersectedFnSignatures<T> = UnionToIntersection<UnionedFnSignatures<T>>;
type UnionToIntersection<T> = InferArg<UnionedFnSignatures<T>>;
type InferArg<ArgFn> = [ArgFn] extends [(arg: infer I) => 1] ? I : never;
type UnionedFnSignatures<T> = T extends T ? (arg: T) => 1 : never;
...
type case5actual = ExtractValuesOfTuple<UnionToTuple<undefined | void | 1>>;
type case11tuple = UnionToTuple<
  "a" | "b" | "c" | 1 | 2 | "d" | "e" | "f" | "g"
>;
type case11actual = ExtractValuesOfTuple<
